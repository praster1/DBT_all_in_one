...
> 이 문서는 책의 **정체성/범위/완료 기준**을 고정해, 챕터별 집필과 실습 자료가 흔들리지 않게 만드는 기준선입니다.
...
| 목표 수준 | 단일 프로젝트 실습 → 멀티 플랫폼 운영/검증/배포 재현 |
...
| 트랙 | 핵심 추적 키 | day1/day2 변화 | 핵심 결과물 |
|---|---|---|---|
| Example A · Retail Orders | `order_id=1001`, `customer_id=10` | 주문 상태/금액/품목 변화 | `fct_orders`, `dim_customers` |
| Example B · Event Stream | `user_id=42`, `session_id=s-100` | 이벤트 유입 증가, 세션 파편화 | `fct_daily_active_users`, `fct_sessions` |
| Example C · Subscription & Billing | `account_id=200`, `subscription_id=sub_01` | 플랜 변경, 상태 전이, 청구 이벤트 반영 | `fct_mrr`, `dim_plans`, snapshot |
...
> ⚠️ **금지**: `로드맵 박스` 기계적 반복, 장마다 예제 리셋, 개념 없는 코드 나열.

## 7) 개정 작업 프로토콜

1. **구조 변경인지, 내용 보강인지 먼저 판정**
2. 구조 변경이면 본문보다 `01_outline/master_toc.md`를 먼저 수정
